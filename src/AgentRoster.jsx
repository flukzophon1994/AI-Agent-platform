    ...
    <div className="agent-card" onClick={() => onOpen(agent.id)} role="button" tabIndex={0}
      ...
      <div className={`card-grad ${color}`}>
        ...
        <div className="card-mid">
          {img ? (
            <img src={img} alt={agent.name} className="card-portrait-img" />
          ) : (
            <div className="card-initials">{initials(agent.name)}</div>
          )}
        </div>
...
export default function AgentRoster({ agents, onOpenAgent, loading, error, lastUpdated, onRefresh }) {
